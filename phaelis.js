...
readdir("./src/events", (err, files) => {
  ...
  files.filter((file) => file.endsWith(".js")).forEach((file) => {
    ...
    console.log(`[Phaelis-EVENT] ${prop.conf.name} yüklendi!`);
  });
});

client.login(config.bot.token)
  .then(() => console.log(`[Phaelis-BOT] Bot ${client.user.username} olarak giriş yaptı!`))
  .catch((err) => console.log(`[Phaelis-BOT] Bot Giriş yapamadı sebep: ${err}`));
